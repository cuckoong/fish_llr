    ...
    for day in days:
        for plate in plates:
            ...
            for period in periods:  # time (minutes) after on/off stimulus
                features_df = get_features(data, period=period, feature_func=feature_func)
                features_df['label'] = labels[labels >= 0]
                features_df.to_csv(os.path.join(result_dir, filename + '-{}-min.csv'.format(period)), index=False)